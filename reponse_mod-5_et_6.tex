...
\subsection{Contexte}
	...
	\frame{
  	...
  	\frame{
		\frametitle{Contexte institutionnel - retour d'expériences des dépistages mis en place dans les autres pays}
		L'Organisation mondiale de la santé (OMS) demande à ce que ...


		Par ailleurs, en 2008, la Commission européenne a mis en place un cadre commun formalisé et l'a mis à jour en 2015.
...
\subsection{\'{E}tat des lieux du dépistage en France}
  	\frame{
		...
		\begin{itemize}
		\item chez certaines femmes, principalement en situation socio-économique aisée~: début du dépistage trop précoce, non respect des intervalles entre 2~dépistages,
		...
		\end{itemize}
		De ce fait, ce taux est en réalité d'environ 55 à 60~\%. Par ailleurs, 40 à 50~\% des femmes concernées qui ne participent pas, ou de façon irrégulière. % p.48
...
\subsection{Quelle population, quelles actions}
	\frame{
		...
		\begin{itemize}
			...
			\item femmes avec risques aggravé de cancer du col (VIH, imunodépression, Distilbène)~;
			\item femmes non sensibilisées (couverture maladie universelle compélmentaire, affection de longue durée, homosexuelles)~;
		...
		\begin{itemize}
			\item stigmatisation des personnes invitées au dépistage (puisqu'appartenant donc aux groupes à risque)~;
...
\subsection{Quelle technique de dépistage mettre en place}
	\frame{
		...
		Cette évaluation a étudié plusieurs stratégies en termes de technique de dépistage et d'intervalle de dépistage~:
		\begin{itemize}
